...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R13_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R13 is a code-only
# fix (masking, read-only verify, diagnostics); config.yaml needs no changes.
...
$files = @(
  ...
  "V243R13_DATAMAP_LEARNING_UNBLOCK_FIX_20260924.md", "APPLY_V243R13_IN_PLACE.ps1", "VERIFY_V243R13_INSTALL.ps1",
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R13 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R13 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R13_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R13 Data Map learning-unblock patch applied in place." -ForegroundColor Green
