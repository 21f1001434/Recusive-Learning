...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R14_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R14 (which includes
# R13) is a code-only fix; config.yaml needs no changes.
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R14 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R14 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R14_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R14 Document Type full-form + self-repair patch (includes R13) applied in place." -ForegroundColor Green
