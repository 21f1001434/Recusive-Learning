...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R17_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R17 (which includes
# R13-R16) is a code-only fix; config.yaml needs no changes. data\hip_memory keeps
# what the agent has learned, including form_structure_memory.
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R17 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R17 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R17_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R17 form variants self-heal + structure learning patch (includes R13-R16) applied in place." -ForegroundColor Green
