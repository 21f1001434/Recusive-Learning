...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R12_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R12 code defaults
# activate new multi-model/continuous-learning fields even when config.yaml is old.
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R12 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R12 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R12_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R12H2 learning-certified patch applied in place." -ForegroundColor Green
