...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R19_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R19 (which includes
# R13-R18) needs no config.yaml changes: the new portal_skills and
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R19 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R19 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R19_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R19 certified skills, input.json operations and fast deterministic replay (includes R13-R18) applied in place." -ForegroundColor Green
