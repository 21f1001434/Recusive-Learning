...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R9_$timestamp"
...
$files = @(
  ...
  "V243R9_PHASE_COMMIT_HANDOFF_FINAL_20260923.md",
  "VERIFY_V243R9_INSTALL.ps1",
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R9 wheel is missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R9 wheel into the active Python environment..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try {
    & python -c "from hip_id_agent.persistent_operator import PersistentHIPOperator; from hip_id_agent.recursive_self_improvement import RecursiveSelfImprovementEngine; print('R9_IMPORT_SMOKE_OK')"
    if ($LASTEXITCODE -ne 0) { throw "R9 import smoke failed" }
    $help = (& python -m hip_id_agent --help 2>&1 | Out-String)
    if ($LASTEXITCODE -ne 0) { throw "hip_id_agent --help failed" }
    if ($help -notmatch "operate-hip") { throw "R9 smoke failed: operate-hip command was not found" }
    Write-Host "R9 smoke check PASS: operate-hip is available." -ForegroundColor Green
  }
  finally { Pop-Location }
}

Write-Host "V243R9 applied in place." -ForegroundColor Green
Write-Host "Target: $TargetRoot"
Write-Host "Backup: $backupRoot"
Write-Host "Preserved: config.yaml, .env, input.json, runs, data\hip_memory, .backend_runtime, .hip_runtime"
Write-Host "Wheel: exact bundled 2.4.3 R9 build installed into active Python unless -SkipWheelInstall was used"
