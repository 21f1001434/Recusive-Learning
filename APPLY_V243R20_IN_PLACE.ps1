...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R20_$timestamp"
...
if (-not $SkipWheelInstall) {
  ...
  Write-Host "Installing exact V243R20 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R20_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R20 portal + rows for every phase (includes R13-R19) applied in place." -ForegroundColor Green
