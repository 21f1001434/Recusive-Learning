...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R15_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R15 (which includes
# R13 and R14) is a code-only fix; config.yaml needs no changes.
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R15 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R15 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R15_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R15 all-phase full-form + phase-aware task box patch (includes R13, R14) applied in place." -ForegroundColor Green
