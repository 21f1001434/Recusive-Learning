...
from .transport_profile_kb import (
    _advance_transport_profile_add_wizard,
    _click_add_transport_profile_with_overlay_recovery,
    _find_add_button,
    _looks_like_transport_profile_add_form,
    build_dummy_fill_values,
    extract_transport_profile_seed,
)


TRANSPORT_PROFILE_URL = PHASE_URLS["source_transport_profile"]
FAMILY = "transport_profiles"
PHASES = ("source_transport_profile", "target_transport_profile")
MUTATION_PROBE_ACTIONS = ("migrate", "deploy", "delete")


def _transport_profile_objects(payload: Mapping[str, Any]) -> Dict[str, Mapping[str, Any]]:
...
def _query_for(obj: Mapping[str, Any]) -> str:
    return str(obj.get("profile_name") or obj.get("transport_profile_name") or obj.get("name") or "").strip()
...
def _field_label(node: Mapping[str, Any]) -> str:
    ...
    return (labels or placeholders or [str(node.get("field_key") or "Transport Profile field")])[0]


def _value_free_transport_profile_blueprint(graph: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "schema_version": "hip.transport_profile-dependency-blueprint.v1",
        "phase": graph.get("phase"),
        "object_family": "transport_profile",
        "nodes": nodes,
        "dependency_edges": edges,
        "repeatable_rows": dict(graph.get("repeatable_rows") or {}),
        "section_order": ["Create Transport Profile"],
        "critical_gates": [
            "System Type exact before System/Partner/Application",
            "Interface Type exact before interface-specific children",
            "Existing Account exact before Account",
            "Account exact before Use Existing Folder",
            "Use Existing Folder exact before Subscription Folder branch",
        ],
        "repeatable_policy": "phase_specific_transport_profile_dependencies",
        "values_stored": False,
    }
...
def _action_api_causal_trace(actions: Sequence[Any], transactions: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    by_id = {str(t.get("request_id") or ""): t for t in transactions if str(t.get("request_id") or "")}
    rows: List[Dict[str, Any]] = []
    linked: set[str] = set()
    for raw in actions:
        a = _event_dict(raw)
        ids = [str(x) for x in (a.get("network_events_triggered") or []) if str(x)]
        txs = []
        for rid in ids:
            t = by_id.get(rid)
            if not t:
            ...
            linked.add(rid)
            txs.append({
                "request_id": rid, "method": t.get("method"), "url": t.get("url"),
                "status": t.get("status"), "stage": t.get("stage"),
                "request_payload_captured": t.get("request_payload") is not None,
                "response_payload_captured": t.get("response_payload") is not None,
            })
        rows.append({
            "action_id": a.get("action_id"), "type": a.get("type"),
            "target": mask_sensitive_string(str(a.get("target") or "")),
            "stage": a.get("stage"), "success": bool(a.get("success", True)),
            "triggered_api_count": len(txs), "triggered_transactions": txs,
            "value_persisted": False,
        })
    return mask_sensitive_data({
        "schema_version": "hip.transport-profile-ui-api-causal-trace.v1",
        "action_count": len(rows), "transaction_count": len(transactions),
        "linked_transaction_count": len(linked),
        "linked_transaction_coverage": round(len(linked) / len(transactions), 4) if transactions else 1.0,
        "actions": rows, "values_stored": False,
    })


class TransportProfileDeepDiscoveryFlow:
    """Deep-learn Transport Profiles without persisting customer values or saving forms.

    The mission learns listing mechanics and row actions, then opens an Add/Create
    Transport Profile surface and executes the existing dependency-aware form runtime
    for both source and target input objects. This yields live evidence for the
    complete Source/Target parent-child hierarchy and interface-specific conditional branches while the
    ...
    async def _close_surface(self, browser: BrowserSession, surface: Mapping[str, Any]) -> bool:
        ...
        for close in _safe_close_candidates(surface):
            ...
            try:
                ...
                await browser.click_and_wait(action="close Transport Profile discovery surface", locator=loc, selector=selector)
    ...
    async def _inspect_filter(self, *, browser: BrowserSession, control: Mapping[str, Any], run_id: str, out_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            await browser.click_and_wait(action=f"inspect Transport Profile filter {label}", locator=loc, selector=selector)
    ...
    async def _restore_searched_expanded(self, *, browser: BrowserSession, query: str, run_id: str, out_dir: Path) -> Dict[str, Any]:
        ...
        if page is None:
            return {"pass": False, "reason": "no page"}
        await browser.goto_base_and_complete_sso(TRANSPORT_PROFILE_URL)
    ...
    async def _probe_mutation_action(self, *, browser: BrowserSession, action: Mapping[str, Any], run_id: str, out_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            installed = True
            browser.set_portal_mutation_authorization(enabled=True, allowed_labels=[label], task_id=f"safe-transport_profile-probe-{_norm(label)}")
            loc = page.locator(selector).first if selector else page.get_by_role("button", name=re.compile(re.escape(label), re.I)).first
            try:
    ...
    async def _open_create_surface(self, *, browser: BrowserSession, run_id: str, out_dir: Path) -> Dict[str, Any]:
        """Open Add/Create Transport Profile safely; abort any unexpected mutation on click."""
        page = browser.page
        if page is None:
            return {"pass": False, "reason": "no page"}
        await browser.goto_base_and_complete_sso(TRANSPORT_PROFILE_URL)
        ...
        fallback_add = None
        if not add:
            fallback_add = await _find_add_button(page)
            if fallback_add is None:
                return {"pass": False, "reason": "Add/Create Transport Profile entry point not found", "surface": surface}
            add = {"label": "Add", "role": "button", "selector": ""}
        ...
        try:
            ...
            browser.set_portal_mutation_authorization(enabled=True, allowed_labels=[label], task_id="safe-open-transport_profile-create")
            loc = fallback_add if fallback_add is not None else (page.locator(selector).first if selector else page.get_by_role("button", name=re.compile(re.escape(label), re.I)).first)
            click_error = ""
            try:
                await browser.click_and_wait(action=label, locator=loc, selector=selector or "Add Transport Profile")
                await asyncio.sleep(0.75)
            except Exception as exc:
                click_error = mask_sensitive_string(str(exc))
            opened = await _looks_like_transport_profile_add_form(page)
            warnings: List[str] = []
            if not opened:
                fresh_add = await _find_add_button(page)
                if fresh_add is not None:
                    opened = await _click_add_transport_profile_with_overlay_recovery(
                        page, browser, fresh_add, kb_dir=out_dir / "open_recovery", warnings=warnings
                    )
            if not opened:
                return {"pass": False, "capability_id": cap["capability_id"], "error": click_error or "Transport Profile Add form did not open", "warnings": warnings, "blocked_requests": blocked}
        except Exception as exc:
    ...
    def _promote_form_topology(self, *, phase: str, graph: Mapping[str, Any], execution: Mapping[str, Any], create_capability_id: str, run_id: str) -> Dict[str, Any]:
        ...
        replay_steps: List[Dict[str, Any]] = [
            {"type": "navigate", "action": "open_transport_profiles", "wait_for": ["listing_ready"]},
            {"type": "action", "action": "open_create", "capability_id": create_capability_id, "scope": "listing_surface", "wait_for": ["transport_profile_form_ready"]},
        ]
        ...
        profile = self.graph.observe_replay_profile(
            ...
            preconditions=["authenticated_shared_browser", "transport_profiles_listing_ready", "create_surface_available", "current_input_present"],
            verified=bool(execution.get("pass")),
            evidence={
                "run_id": run_id, "phase": phase, "repeatable_policy": "phase_specific_transport_profile_dependencies",
    ...
    async def _exercise_create_form(self, *, browser: BrowserSession, payload: Dict[str, Any], phase: str, run_id: str, out_dir: Path) -> Dict[str, Any]:
        ...
        if not opened.get("pass"):
            ...
            return opened
        # The HIP Transport Profile create flow can initially expose only System Type.
        # Expand the wizard using the exact current phase values, then let the shared
        # stateful runtime verify/repair every field. The expansion never clicks
        # Save/Create/Submit and its values are kept only in run evidence.
        phase_payload = dict(payload)
        phase_payload["_full_dummy_fill_phase"] = phase
        seed = extract_transport_profile_seed(phase_payload, phase=phase)
        wizard_values = build_dummy_fill_values(seed, exact=True)
        wizard_warnings: List[str] = []
        try:
            wizard_controls = await _advance_transport_profile_add_wizard(
                browser.page, browser, kb_dir=out_dir / "wizard_expand", warnings=wizard_warnings, dummy_values=wizard_values
            )
        except Exception as exc:
            wizard_controls = []
            wizard_warnings.append(mask_sensitive_string(str(exc)))
        safe_write_json(out_dir / "wizard_expand" / "summary.json", {
            "control_count": len(wizard_controls),
            "warnings": mask_sensitive_data(wizard_warnings),
            "values_persisted_to_capability_memory": False,
        })
        ...
        blueprint = _value_free_transport_profile_blueprint(graph)
        ...
        action_start = len(browser.action_events)
        ...
        causal_trace = _action_api_causal_trace(browser.action_events[action_start:], network.get("transactions") or [])
        safe_write_json(out_dir / "api" / "ui_api_causal_trace.json", causal_trace)
        ...
        result = {
            ...
            "wizard_expand": {"control_count": len(wizard_controls), "warnings": mask_sensitive_data(wizard_warnings)},
    ...
    async def run(self, *, ctx: RunContext, input_json: str | Path) -> Dict[str, Any]:
        ...
        objects = _transport_profile_objects(payload)
        root = Path(ctx.run_dir)
        out = root / "deep_discovery" / FAMILY
        out.mkdir(parents=True, exist_ok=True)
        agentq = HIPAgentQController(memory_root=Path(self.config.reporting.memory_dir), run_dir=root / "agentq_transport_profile_deep", config=self.config)
        summary: Dict[str, Any] = {
            "schema_version": "hip.transport_profile-deep-discovery.v1", "run_id": ctx.run_id,
        ...
        async with BrowserSession(self.config, root) as browser:
            ...
            try:
                browser._active_phase_name = "deep_discovery_transport_profiles"
                browser._current_stage = "deep_discovery:transport_profiles:open"
                await browser.goto_base_and_complete_sso(TRANSPORT_PROFILE_URL)
                await browser.wait_ready()
                ...
                # Search/row-action learning for BOTH source and target Transport Profiles.
                for phase in PHASES:
        ...
        safe_write_json(root / "transport_profile_deep_discovery_summary.json", summary)
        self.graph.save()
