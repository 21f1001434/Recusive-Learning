...
$backupRoot = Join-Path $TargetRoot ".hip_patch_backups\V243R16_$timestamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null

# User/environment/runtime state is intentionally preserved. R16 (which includes
# R13, R14 and R15) is a code-only fix; config.yaml needs no changes.
...
if (-not $SkipWheelInstall) {
  if (-not (Test-Path $wheel)) { throw "R16 wheel missing after patch copy: $wheel" }
  Write-Host "Installing exact V243R16 wheel..." -ForegroundColor Cyan
...
if (-not $SkipSmokeCheck) {
  ...
  try { & .\VERIFY_V243R16_INSTALL.ps1 } finally { Pop-Location }
}
Write-Host "V243R16 watchdog heartbeat + verification verdict patch (includes R13-R15) applied in place." -ForegroundColor Green
